...
export const IntrestApply = async () => {
  const loanIdList = await EmiLoan.find({ loanStatus: true }, "_id").lean();

  loanIdList.forEach(async (id) => {
    const loanDetail = await EmiLoan.findOne({ _id: id });
    const loanDate = new Date(loanDetail.loanDate);
    const todayDate = new Date();

    const difference = Math.abs(loanDate.getMonth() - todayDate.getMonth());

    if (numberOfDepositEmis < difference) {
      const transaction = {
        desc: "Intrest Apply",
        credit: 0,
        debit:
          (-loanDetail.passbook[loanDetail.passbook.length - 1].balance % 100) *
          loanDetail.intrestRate,
        balance:
          loanDetail.passbook[loanDetail.passbook.length - 1].balance -
          (loanDetail.passbook[loanDetail.passbook.length - 1].balance % 100) *
            loanDetail.intrestRate,
      };
